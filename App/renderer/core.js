...
const NUOVO = () => ({
  ...
  concordati:null,
/* Le rate di ogni sezione, una lista per ognuna: «rate[idSezione]». Nascono
     al bisogno con le due scadenze di sempre (vedi «rateSezione»), e il
     preventivo complessivo non ne ha di proprie — le somma dalle sezioni. */
  rate:{},
...
/* ------------------------------------------------------------------
   Le rate: una lista per OGNI sezione.

   Aggiungerne una in «Cucina» non la fa comparire in «Posa»: sono preventivi
   diversi, con scadenze diverse. Il preventivo complessivo non ha rate sue —
   le ricava dalle sezioni, e proprio per questo i suoi conti sono sempre la
   somma di quello che si incassa davvero.

   L'ordine dice il ruolo: la PRIMA è l'acconto, l'ULTIMA è il rimanente. Le
   rate aggiunte si infilano in mezzo, e nel complessivo si sommano per
   posizione: le prime con le prime, le ultime con le ultime, le altre a
   scendere dall'alto.
...
function rateSezione(tabId){
  if(!S.rate || typeof S.rate!=='object' || Array.isArray(S.rate)) S.rate={};
  if(!Array.isArray(S.rate[tabId])) S.rate[tabId]=RATE_INIZIALI();
  return S.rate[tabId];
}

/* In quale riga del complessivo finisce la rata «j» di una sezione che ne ha
   «quante», quando il complessivo di righe ne ha «righe». La prima resta la
   prima, l'ultima resta l'ultima — il rimanente è il rimanente — e quelle in
   mezzo si allineano dall'alto. */
function rigaNelTotale(j,quante,righe){
  if(quante<=1) return 0;
  if(j===quante-1) return righe-1;
  return Math.min(j,righe-2);
}
/* e la strada all'indietro: quale rata di una sezione corrisponde alla riga
   «k» del complessivo. Torna -1 se quella sezione quella riga non ce l'ha. */
function rataDallaRiga(k,quante,righe){
  if(quante<=0) return -1;
  if(quante<=1) return k===0?0:-1;
  if(k===righe-1) return quante-1;
  return k<quante-1 ? k : -1;
}

/** le righe del complessivo: nome, percentuale e importo, ricavati dalle sezioni */
function rateComplessivo(){
  const sezioni=tabNelTotale();
  const righe=sezioni.reduce((m,t)=>Math.max(m,rateSezione(t.id).length),0);
  if(!righe) return [];
  const importi=new Array(righe).fill(0), nomi=new Array(righe).fill('');
  for(const t of sezioni){
    const rate=rateSezione(t.id), base=baseAcconti(t.id);
    rate.forEach((x,j)=>{
      const k=rigaNelTotale(j,rate.length,righe);
      importi[k]=cent(importi[k]+cent(base*numIT(x.perc)/100));
      if(!nomi[k]) nomi[k]=x.nome;
    });
  }
  const base=baseAcconti(ID_TOT);
  return importi.map((imp,k)=>({
    id:'tot'+k, nome:nomi[k]||('RATA '+(k+1)),
    perc: base?percPiena(imp*100/base):'0', importo:imp, fissa:false,
  }));
}

/** le rate di un foglio, con l'importo già fatto */
function accontiDelFoglio(tabId){
  if(tabId===ID_TOT) return rateComplessivo();
  const base=baseAcconti(tabId);
  return rateSezione(tabId).map(x=>({
    id:x.id, nome:x.nome, perc:x.perc, fissa:!!x.fissa,
    importo:cent(base*numIT(x.perc)/100),
  }));
}
...
/* ---- le operazioni su una sezione ---- */

/* Una rata nuova si infila PRIMA dell'ultima: l'ultima è il rimanente, e il
   rimanente sta in fondo. Nasce a zero, e il conto continua a fare 100 finché
   non le si dà un valore. */
function aggiungiRataIn(tabId){
  const rate=rateSezione(tabId);
  const dove=Math.max(1,rate.length-1);
  rate.splice(dove,0,{id:uid(), nome:'ACCONTO', perc:''});
  return dove;
}
function togliRataIn(tabId,id){
  const rate=rateSezione(tabId);
  const i=rate.findIndex(x=>x.id===id);
  if(i<0) return false;
  rate.splice(i,1);
  pareggiaRate(tabId,-1);
  return true;
}
/* «comando» distingue chi ha scritto. In una sezione comanda l'utente su quella
   rata, e le rate che aveva già deciso a mano restano dove sono. Dal
   complessivo invece si detta una regola per tutti — «il primo acconto è il
   40%» — e allora le sezioni si rifanno i conti per intero: una sezione che
   restasse indietro darebbe un totale che non torna. */
function scriviPercIn(tabId,i,valore,comando){
  const rate=rateSezione(tabId);
  if(!rate[i]) return;
  rate[i].perc=valore;
  rate[i].fissa=true;                 // scritta a mano: il pareggio non la sposta
  pareggiaRate(tabId,i,comando);
}

/* ---- e quelle date dal complessivo, che valgono per tutte le sezioni ---- */

/* Dal complessivo si comanda su tutte le sezioni che ci stanno dentro: quello
   che si scrive lì è la somma di tutte, e tutte devono seguirlo. Le sezioni
   tenute fuori dal complessivo non si toccano: sono fuori anche da questo. */
function perOgniSezione(fn){ for(const t of tabNelTotale()) fn(t.id); }

function aggiungiRataOvunque(){ perOgniSezione(id=>aggiungiRataIn(id)); }

function scriviPercDalTotale(k,valore){
  const righe=rateComplessivo().length;
  perOgniSezione(id=>{
    const rate=rateSezione(id);
    const j=rataDallaRiga(k,rate.length,righe);
    if(j>=0) scriviPercIn(id,j,valore,true);   // detta dal complessivo: vale per tutti
  });
}
function rinominaDalTotale(k,nome){
  const righe=rateComplessivo().length;
  perOgniSezione(id=>{
    const rate=rateSezione(id);
    const j=rataDallaRiga(k,rate.length,righe);
    if(j>=0) rate[j].nome=nome;
  });
}
function togliRataDalTotale(k){
  const righe=rateComplessivo().length;
  perOgniSezione(id=>{
    const rate=rateSezione(id);
    const j=rataDallaRiga(k,rate.length,righe);
    if(j>=0){ rate.splice(j,1); pareggiaRate(id,-1); }
  });
}

/* I preventivi salvati prima avevano UN elenco di rate solo, uguale per tutte
   le sezioni, con le percentuali della singola sezione annotate a parte. Qui
   diventano le rate di ognuna: gli stessi numeri, sistemati dove vivono ora. */
function migraRate(doc){
  if(doc.rate && typeof doc.rate==='object' && !Array.isArray(doc.rate)) return doc.rate;
  const vecchie = Array.isArray(doc.acconti) ? doc.acconti : null;
  const rate={};
  for(const t of (doc.tabs||[]).filter(t=>t.type==='voci')){
    rate[t.id] = vecchie
      ? vecchie.map(a=>({id:uid(), nome:a.nome,
...
function eliminaTab(id){
  ...
  const fai=()=>{
    ...
    if(S.rate) delete S.rate[id];              // le rate della sezione se ne vanno con lei
...
/* La riga negli appunti dell'app. Non sono gli appunti di Windows: lì dentro
   ci finisce del testo, mentre qui serve la riga intera — articolo,
   descrizione, prezzo, quantità, IVA, promo — per rimetterla giù uguale anche
   in un'altra sezione. Vive finché l'app è aperta. */
let rigaCopiata=null;

function copiaRiga(id){
  const r=rigaById(id); if(!r) return;
  const {id:_via, tabId:_pure, ...dati} = r;    // id e sezione se li prende chi incolla
  rigaCopiata=dati;
  toast('Riga copiata: «'+(r.articolo||r.descrizione||'senza nome')+'»','ok');
}

/* Incolla sotto alla riga su cui si è premuto, dentro la SUA sezione: è lì che
   si sta guardando. Da una sezione all'altra funziona uguale — la riga non si
   porta dietro la sezione di provenienza, la prende da dove atterra. */
function incollaRiga(id){
  if(!rigaCopiata) return;
  const rif=rigaById(id);
  const tabId = rif ? rif.tabId : (S.active!==ID_TOT ? S.active : (tabVoci()[0]||{}).id);
  if(!tabId) return toast('Non c\'è una sezione dove incollarla','err');
  const nuova=Object.assign({},rigaCopiata,{id:uid(),tabId});
  const dove = rif ? S.righe.indexOf(rif)+1 : S.righe.length;
  S.righe.splice(dove,0,nuova);
  tocca(); render();
  $(`#corpo tr[data-id="${nuova.id}"]`)?.scrollIntoView({block:'nearest'});
  const sez=tabById(tabId);
  toast('Riga incollata'+(sez&&S.active!==tabId?' in «'+sez.name+'»':''),'ok');
  return nuova;
}
...
function duplicaTab(id){
  ...
  /* anche le rate: sono sue, e una copia deve nascere identica all'originale */
  S.rate[copia.id]=rateSezione(id).map(x=>Object.assign({},x,{id:uid()}));
...
function totaliHTML(T){
  ...
  return `
    <div class="totali">
      ...
      <div class="acconti">${accontiHTML()}</div>
    </div>
    ${nota?`<div class="notaconc">${esc(nota)}</div>`:''}
    ${notaPercentuali()}`;
}

/* Il riquadro delle rate. Il tasto per aggiungerne una sta subito sotto alla
   prima: le rate nuove nascono lì, in mezzo, e il rimanente resta in fondo dove
   deve stare.
   Nel complessivo si scrive lo stesso — nome e percentuale — ma quello che si
   scrive vale per tutte le sezioni: è la somma di tutte. L'importo no: quello
   è un conto, e si cambia nella sezione che lo determina. */
function accontiHTML(){
  const inSezione = S.active!==ID_TOT;
  const rate=accontiDelFoglio(S.active);
  const titoloPerc = inSezione
    ? 'Percentuale di questa rata in questa sezione'
    : 'Percentuale sul totale: scrivendola qui la seguono tutte le sezioni';
  const riga=(x,i)=>`
    <div class="rt acconto${x.fissa?' ritoccata':''}" data-i="${i}">
      <input class="nome" value="${esc(x.nome)}" placeholder="Descrizione della rata" title="${esc(x.nome)}">
      <span class="pc"><input class="perc" value="${esc(percCorta(x.perc))}" inputmode="decimal" placeholder="0"
        title="${esc(titoloPerc+' — esatta: '+percEsatta(x.perc)+'%')}">%</span>
      ${inSezione
        ? `<span class="v scrivi"><input class="imp" value="${esc(fmtNum(x.importo))}" inputmode="decimal"
...
function notaPercentuali(){
  ...
  const somma=cent(rate.reduce((s,x)=>s+numIT(x.perc),0));
...
function legaConcordato(){
  ...
  /* Rate e acconti. In una sezione si scrive sulle sue rate; nel complessivo
     quello che si scrive vale per tutte le sezioni, perché lì i numeri sono la
     loro somma. Come per il prezzo concordato, scrivendo non si ridisegna il
     riquadro: si riscrivono solo gli importi. */
  const inSezione = S.active!==ID_TOT;

  $('#btnAcconto')?.addEventListener('click',()=>{
    /* Dal complessivo la rata si aggiunge a TUTTE le sezioni: è l'unico modo
       perché quella riga in più abbia un senso, visto che il complessivo non
       fa che sommarle. */
    if(inSezione) aggiungiRataIn(S.active); else aggiungiRataOvunque();
    tocca(); aggiornaTotali();
    /* la rata nuova è la seconda: è lì che si va a scrivere il nome */
    $$('#riquadroTotali .rt.acconto')[1]?.querySelector('.nome')?.select();
  });

  $$('#riquadroTotali .rt.acconto').forEach(el=>{
    const i=+el.dataset.i;
    if(!accontiDelFoglio(S.active)[i]) return;
    const nome=el.querySelector('.nome'), perc=el.querySelector('.perc'), imp=el.querySelector('.imp');

    nome.oninput=()=>{
      if(inSezione) rateSezione(S.active)[i].nome=nome.value;
      else rinominaDalTotale(i,nome.value);
      nome.title=nome.value; tocca();
    };

    /* La percentuale: in una sezione tocca alla sua rata, dal complessivo a
       quella corrispondente di ogni sezione. Poi, dentro ogni sezione, le
       altre rate si aggiustano da sé per fare 100. */
    const scritta=valore=>{
      if(inSezione) scriviPercIn(S.active,i,valore);
      else scriviPercDalTotale(i,valore);
    ...
    perc.onblur=()=>{
      ...
      const piena=accontiDelFoglio(S.active)[i]?.perc ?? '';
    ...
    if(imp){
      const daImporto=()=>{
        ...
        scriviPercIn(S.active,i,percPiena(Math.max(0,Math.min(100,quota))));
        riscriviPercentuali(null);      // si scrive nell'importo: le percentuali si riscrivono tutte
        tocca(); ricalcolaAcconti(imp);
      };
      imp.oninput=daImporto;
      imp.onblur=()=>{ daImporto(); imp.value=fmtNum(accontiDelFoglio(S.active)[i]?.importo||0); };
    }

    el.querySelector('[data-act="delacc"]').onclick=()=>{
      const x=accontiDelFoglio(S.active)[i];
      if(inSezione) togliRataIn(S.active,x.id); else togliRataDalTotale(i);
      tocca(); aggiornaTotali();
    };
  });
}

/* Le rate di una sezione devono fare 100. Toccandone una, a pareggiare il
   conto si muove UNA SOLA delle altre: l'ultima, che è la rata del saldo — «il
   rimanente» — e se lei non basta si risale a quella sopra.

   Le rate su cui si è scritto non si toccano: la loro cifra è una decisione,
   non un calcolo. Se sono state scritte tutte non c'è dove mettere la
...
function pareggiaRate(tabId,escluso,comando){
  const rate=rateSezione(tabId), n=rate.length;
  if(n<2) return;
  const val = k => Math.max(0,numIT(rate[k].perc));
  let differenza = 100 - rate.reduce((somma,_,k)=>somma+val(k),0);
  for(let k=n-1; k>=0 && Math.abs(differenza)>1e-9; k--){
    if(k===escluso) continue;
    if(rate[k].fissa && !comando) continue;        // scritta a mano: non si tocca
    const attuale=val(k);
    const nuovo=Math.max(0,attuale+differenza);
    if(nuovo===attuale) continue;                  // già a zero e c'è da togliere: si sale
    rate[k].perc=percPiena(nuovo);
    /* mossa da un comando del complessivo, quella cifra non è più quella
       scritta a mano: smette di essere intoccabile */
    if(comando) rate[k].fissa=false;
    differenza-=(nuovo-attuale);
  }
}

/* Riporta nei campi le percentuali ricalcolate, tranne quello in cui si sta
   scrivendo: corta per leggerla, con quella intera nel suggerimento. */
function riscriviPercentuali(escluso){
  const rate=accontiDelFoglio(S.active);
  $$('#riquadroTotali .rt.acconto').forEach(el=>{
    const x=rate[+el.dataset.i]; if(!x) return;
    const inp=el.querySelector('.perc');
    if(!inp || inp===escluso) return;
    const corta=percCorta(x.perc);
    if(inp.value!==corta) inp.value=corta;
    inp.title=inp.title.replace(/ — esatta: .*$/,'')+' — esatta: '+percEsatta(x.perc)+'%';
  });
}
...
function ricalcolaAcconti(escluso){
  const rate=accontiDelFoglio(S.active);
  $$('#riquadroTotali .rt.acconto').forEach(el=>{
    const x=rate[+el.dataset.i];
    if(!x) return;
    const imp=el.querySelector('.imp');
    if(imp){ if(imp!==escluso) imp.value=fmtNum(x.importo); }
    else el.querySelector('.v').textContent=fmtEur(x.importo);
    el.classList.toggle('ritoccata',!!x.fissa);
  });
...
/* Le rate di una sezione sono sue e basta: togliendola dal complessivo non c'è
   più niente da staccare, e rimettendola dentro niente da riattaccare — il
   complessivo semplicemente smette di contarla, o ricomincia. */
  ...
  if(rate.length){
    ...
    for(const a of rate){
      ...
      add(cellaTesto(1,r,a.nome,23),
          cellaNum(5,r,numIT(a.perc),24),
          cellaForm(6,r,`${RIF(6,rBase)}*${RIF(5,rp)}/100`,a.importo,29));
...
function caricaDocumento(testo,percorso){
  ...
  S.rate=migraRate(doc);          // dall'elenco unico alle rate di ogni sezione
  delete S.acconti;               // il vecchio elenco ha finito il suo lavoro
...
/* ==============================================================================
   IMPORTA UN ALTRO PREVENTIVO

   Si sceglie un preventivo salvato e si decide che cosa farne:

   «Implementa» lo porta dentro a questo — una sezione nuova per ogni sezione
   che ha, con le sue voci e le sue rate — e il preventivo aperto resta dov'è,
   solo più grande. Serve quando un lavoro nasce dall'unione di due preventivi
   già fatti.

   «Nuova Istanza» lo apre in un'altra finestra dell'app, che vive per conto
   suo: quando i due preventivi devono restare due, ma li si vuole guardare
   insieme.
   ============================================================================== */
async function dialogoImporta(){
  if(!IO.desktop) return toast('Vale per l\'app installata','');
  let f=null;
  try{ f=await IO.apri(); }catch(e){ return toast('Apertura non riuscita: '+e.message,'err'); }
  if(!f) return;                                  // ha annullato la scelta del file

  let doc=null;
  try{
    const d=JSON.parse(f.contenuto);
    doc=d.documento||d;
    if(!doc||!Array.isArray(doc.tabs)) throw new Error('File non riconosciuto');
  }catch(e){ return toast('Non è un preventivo che so leggere: '+e.message,'err'); }

  const sezioni=doc.tabs.filter(t=>t.type==='voci');
  const righeDelDoc=Array.isArray(doc.righe)?doc.righe:[];
  const quante=t=>righeDelDoc.filter(r=>r.tabId===t.id).length;
  const nome=doc.nome||nomeDaPercorso(f.percorso);

  modal('Importa',`
    <p style="margin:0">Da <b>${esc(nome)}</b>:</p>
    <div class="cartelle">
      ${sezioni.length
        ? sezioni.map(t=>`<div class="cartriga"><span class="k">${esc(t.icon||'📄')} ${esc(t.name)}</span><span class="v">${quante(t)} voci</span></div>`).join('')
        : `<div class="cartriga"><span class="k">Nessuna sezione</span><span class="v"></span></div>`}
    </div>
    <p class="hint"><b>Implementa</b> aggiunge queste sezioni al preventivo aperto, con le loro
    voci e le loro rate: quello che hai adesso resta dov'è.<br>
    <b>Nuova Istanza</b> lo apre in un'altra finestra, che lavora per conto suo.</p>
  `,
  [{label:'Implementa',primary:true,fn:()=>{ implementaDocumento(doc,nome); }},
   {label:'Nuova Istanza',fn:async()=>{
      const r=await IO.nuovaIstanza?.(f.percorso);
      if(r&&r.errore) toast('Non riesco ad aprire un\'altra finestra: '+r.errore,'err');
      else toast('Aperto in un\'altra finestra','ok');
   }},
   {label:'Chiudi'}],
  null,'conx');
}

/* Le sezioni del preventivo scelto entrano in questo, una per una. Gli
   identificativi si rifanno tutti: due preventivi nati dallo stesso file
   avrebbero le stesse sigle, e le righe finirebbero nella sezione sbagliata. */
function implementaDocumento(doc,nome){
  const sezioni=doc.tabs.filter(t=>t.type==='voci');
  if(!sezioni.length) return toast('Quel preventivo non ha sezioni da portare dentro','err');
  const righeDelDoc=Array.isArray(doc.righe)?doc.righe:[];
  const rateDelDoc=migraRate(doc);
  let voci=0;
  for(const t of sezioni){
    const nuovo={id:'s'+uid(), type:'voci', name:t.name, icon:t.icon||'📄'};
    S.tabs.push(nuovo);
    if(concordatoAttivo()) S.concordati[nuovo.id]='';
    for(const r of righeDelDoc.filter(x=>x.tabId===t.id)){
      S.righe.push(Object.assign({},r,{id:uid(), tabId:nuovo.id}));
      voci++;
    }
    const rate=rateDelDoc[t.id];
    S.rate[nuovo.id] = Array.isArray(rate)
      ? rate.map(x=>Object.assign({},x,{id:uid()}))
      : RATE_INIZIALI();
    S.active=nuovo.id;
  }
  S.righe=migraPromo(S.righe);
  tocca(); render();
  toast('Portate dentro '+sezioni.length+(sezioni.length===1?' sezione':' sezioni')+
        ' da «'+nome+'» ('+voci+(voci===1?' voce':' voci')+')','ok');
}
...
function contestoDaEvento(e){
  ...
  if(tr){
    ...
    return {tipo:'riga', id:tr.dataset.id, barrato:!!(r&&r.barrato), copiato:!!rigaCopiata};
  }
  return {tipo:'testo'};
}
...
function eseguiAzioneMenu(azione,id){
  ...
  if(azione==='copiaRiga')   copiaRiga(id);
  if(azione==='incollaRiga') incollaRiga(id);
...
function menuDom(ctx,x,y){
  ...
  const voci = ctx.tipo==='tab'
    ...
    : [['Duplica Riga','duplicaRiga'],['Copia Riga','copiaRiga'],
       ...(rigaCopiata?[['Incolla Riga','incollaRiga']]:[]),
       [ctx.barrato?'Togli la barratura':'Promo','promo'],['Elimina','eliminaRiga']];
  const m=document.createElement('div');
  m.className='menuctx';
