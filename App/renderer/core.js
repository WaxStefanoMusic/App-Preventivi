...
/* Le rate devono fare 100. Toccandone una, a pareggiare il conto si muove UNA
   SOLA delle altre: l'ultima, che è la rata del saldo — «il rimanente». Se è
   proprio lei quella che si sta scrivendo, tocca alla penultima.

   Prima la differenza si spalmava su tutte in proporzione, e con tre rate
   succedeva questo: 4.000 sulla prima, 100 sulla seconda, e la terza — che
   valeva zero e quindi non pesava — restava a zero mentre la prima schizzava a
   7.900. Le cifre già decise a mano non si toccano: si muove il saldo, che è
   lì per quello. Con due rate o con cento la regola non cambia. */
function bilanciaAcconti(i,tabId){
  if(S.acconti.length<2) return;                 // con una rata sola non c'è niente da bilanciare
  pareggiaAcconti(tabId,i);
}

/* Rimette il conto a 100 partendo dal basso: la differenza la prende la rata
   del saldo — l'ultima — e se lei non basta si risale a quella sopra, e così
   via. La rata che si sta scrivendo non si tocca mai, e nessuna scende
   sottozero.

   Serve proprio questo: con tre rate, 4.000 sulla prima e 100 sulla seconda,
   la terza deve trovarsi 3.900 da sé — non la prima a schizzare a 7.900. E con
   otto rate, se quella in fondo è già a zero e non può assorbire, la
...
function pareggiaAcconti(tabId,escluso){
  ...
  for(let k=n-1; k>=0 && Math.abs(differenza)>1e-9; k--){
    ...
    impostaPerc(S.acconti[k],tabId,percPiena(nuovo));
...
/* Tolta una rata, il conto lo pareggia il saldo, con la stessa regola: le
   altre restano quelle che erano. Ridistribuendo in proporzione cambierebbero
   tutte, e chi aveva scritto «4.000 alla conferma» se lo ritroverebbe diverso
   per aver cancellato tutt'altra riga. */
function normalizzaIn(tabId){
  if(!S.acconti.length) return;
  pareggiaAcconti(tabId,-1);
}
