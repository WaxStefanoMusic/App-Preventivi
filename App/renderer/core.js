...
function righeDi(tabId){
  if(tabId===ID_TOT){
    ...
    return S.righe.slice().sort((a,b)=>(ordine.get(a.tabId)??99)-(ordine.get(b.tabId)??99));
  }
...
const conSezioniAMano = () => tabVoci().some(t=>sezioneRitoccata(t.id));
...
function sommaConcordati(){
  if(!concordatoAttivo()) return;
  let s=0;
  for(const t of tabVoci()) s=cent(s+numIT(valConcordato(t.id)));
...
function renderTabs(){
  ...
  S.tabs.forEach(t=>{
    const b=document.createElement('button');
    b.className='tab'+(t.id===S.active?' active':'');
  ...
  const corpo = righe.length
    ? righe.map(r=>{
        ...
        return `<tr${p?' class="conpromo"':''}>
          ...
          <td class="num">${due(r.prezzo===''?'':fmtNum(numIT(r.prezzo)), String(r.promo??'').trim()===''?'':fmtNum(numIT(r.promo)))}</td>
